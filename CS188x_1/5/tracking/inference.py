...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            for p in self.legalPositions: self.beliefs[p] = 0.0
            self.beliefs[self.getJailPosition()] = 1.0
            return
        for position in self.legalPositions:
            manhattanDistance = abs(pacmanPosition[0] - position[0]) + abs(pacmanPosition[1] - position[1])
            # print position, 'manhattan distance for each legal position'
            # print pacmanPosition, 'pacman position'
            if manhattanDistance in emissionModel:
                self.beliefs[position] = emissionModel[manhattanDistance]*self.beliefs[position]
            else:
                self.beliefs[position] = 0.0
            self.beliefs.normalize()
        return
        # print observation, 'noisy distance'
        # print pacmanPosition, 'pacman position'
        # print emissionModel, 'emission model'
        # print sum(emissionModel.values()), 'sum of emission models'
        # print self.legalPositions, 'legal positions'
        # print self.getJailPosition(), 'jail position of the ghost'
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in legalPositions: newBeliefs[p] = 0.0
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        cycle = itertools.cycle(self.legalPositions)
        self.particles = map(lambda x: cycle.next(), range(self.numParticles))
        # weights = [1.0]*len(self.legalPositions)
    ...
    def observe(self, observation, gameState):
        ...
        emissionModel = busters.getObservationDistribution(noisyDistance)
        pacmanPosition = gameState.getPacmanPosition()
        particleWeights = map(lambda x: emissionModel[util.manhattanDistance(x, pacmanPosition)],\
            self.particles)
        if sum(particleWeights) == 0.0:
            self.initializeUniformly(gameState)
            return
        self.particles = util.nSample(particleWeights, self.particles, self.numParticles)
        return
    ...
    def getBeliefDistribution(self):
        ...
        "*** YOUR CODE HERE ***"
        belief = util.Counter()
        for position in set(self.legalPositions + self.particles):
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.states = list(itertools.product(*[self.legalPositions]*self.numGhosts))
        random.shuffle(self.states)
        # print self.states
        cycle = itertools.cycle(self.states)
        self.particles = map(lambda x: cycle.next(), range(self.numParticles))
        # print self.particles
        # self.particles = util.nSample([1.0]*len(self.states), self.states, self.numParticles)
        # for i in self.capturedGhosts:
        #     self.particles = map(lambda x: self.getParticleWithGhostInJail(x, i), self.particles)
        # print 'time taken in init', time.time() - t
    ...
    def observeState(self, gameState):
        ...
        for i, nD in enumerate(noisyDistances):
            if nD == None:
                if self.capturedGhosts.count(i) == 0:
                    self.capturedGhosts.append(i)
        particleProbs = []
        weights = []
        for particle in self.particles:
            particle_distances = map(lambda x: util.manhattanDistance(x, pacmanPosition), particle)
            particle_prob = []
            weight = 1.0
            for i in range(len(noisyDistances)):
                if noisyDistances[i] == None:
                    ...
                    weight *= emissionModels[i][particle_distances[i]]
        ...
        for i in self.capturedGhosts:
            self.particles = map(lambda x: self.getParticleWithGhostInJail(x, i), self.particles)
        # print 'time taken is in observation', time.time() - t
        "*** YOUR CODE HERE ***"
    ...
    def getBeliefDistribution(self):
        ...
        for i in self.particles:
            belief[i] += 1 #self.particles.count(i)
        belief.normalize()
        # print 'time taken in getting belief', time.time() - t
        return belief
        util.raiseNotDefined()
